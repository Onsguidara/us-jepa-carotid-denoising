...
## Deployment: n8n Denoising Agent

Beyond the research notebooks, this repo includes a deployable conversational agent (`agent/`) that wraps the trained denoising model behind a FastAPI backend and an n8n workflow. Users can send an ultrasound image and receive back the denoised result, quality metrics (noise proxy, edge energy, inference time), and a natural-language explanation generated by an LLM agent.

See [`agent/README.md`](agent/README.md) for setup and usage instructions.
